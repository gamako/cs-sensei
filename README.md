...
命令とメモリ階層、GC、トランザクション、並行性、分散、圧縮、テスト設計、評価設計。
レイヤーも分野も問いません。ただし教科書の目次からは選びません。
...
**手元のファイルを読み書きできるエージェント**が要ります。
Claude Code、Codex、GitHub Copilot のいずれか。

**ブラウザやチャット画面で動く Claude・ChatGPT、Cowork ではできません。**
学習記録も課題のコードも手元のファイルとして残るので、ファイルを触れないと始まりません。

どれを使うか決まっていなければ、**VS Code の拡張から始めるのが簡単です。**
エディタの中でエージェントが動いて、ファイルが左に見えている状態になります。
...
**git を使ったことがなければ、リポジトリにしなくてかまいません。** こう頼めば済みます。

```
https://github.com/gamako/cs-sensei の template/ の中身を、いま開いている場所に置いて、
AGENTS.md を読んで、そのまま学習を始めて
```

**フォルダも作らなくて大丈夫です。** デスクトップでもダウンロードでも、いま開いている
ところでそのまま始められます。記録も課題も手元のファイルとして残るので、これだけで
続きます。**git は後から足せます。** 必要になってからエージェントに頼めば移せます。
